...
| `fetch-altsignal` | 🟡 | Phase 5 alt-signal: DefiLlama (chain TVL, stablecoin supply, DEX volume), Kalshi and Polymarket (hand-picked markets from `cfg/altsignal.yaml`). Same tier as `fetch-funding` — free, third-party, rate-limitable — and read by `review` above. `--report` summarises what's logged. pump.fun is deliberately absent; see `scripts/probe_pumpfun_migrations.py` for why nothing free and keyless answers "what recently graduated" yet. |
