...
| `dashboard` | 🟢 | The #84/#85 browser Surface: a JSON API over `review.cli.load_books` (ADR-0011's own wire shape, never a View's result object) plus the built `web/` front end. Binds `127.0.0.1` only, with no flag to widen it, and imports neither `execution` nor `llm` (ADR-0010) — it cannot spend and cannot sign. `--print-openapi` dumps the schema and exits without binding a port, which is how `scripts/gen-api-types.sh` regenerates `web/src/api/schema.d.ts` without a running server. |
