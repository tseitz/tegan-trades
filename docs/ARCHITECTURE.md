...
| `fetch-altsignal` | 🟡 | Phase 5 alt-signal: DefiLlama (chain TVL, stablecoin supply, DEX volume), Kalshi and Polymarket (hand-picked markets from `cfg/altsignal.yaml`), and pump.fun (recently graduated tokens, via Solana Tracker — needs `SOLANATRACKER_API_KEY`, see `.env.example`; skipped with a message if unset). Same tier as `fetch-funding` — free, third-party, rate-limitable — and read by `review` above for the first three. pump.fun is logged but **not** surfaced in `review`: a brand-new graduating token confirms no holding or roster thesis, so it stays report-only (`--report`). |
