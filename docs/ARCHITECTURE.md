...
`data/` is regenerable ore, and no stage mutates what an upstream stage wrote — **with three
...
and `data/interest/` hold observations of a window the venues serve only as a live snapshot —
see `oracle.funding_store` and `oracle.interest_store`. Deleting any of them loses information
permanently. Everything else can be rebuilt by re-running its stage.
...
| `digest` | 🔴 | What changed overnight. One `claude -p` call for the roster section only — ~17s, trivial. Everything else is local file reads, now including a `review` pass over every portfolio in `data/portfolios/` — verdict moves plus positions that reached a weekly level overnight (free; a portfolio that fails to load costs a warning, not the digest) — and a `treasury` view read (#75): parked money, standing state, plus any gate-clearing yield opportunity idle cash could move to, reported once and then only as a count. **Net worth (#76)** sums those same two reads — every portfolio's priced holdings plus its cash, plus Treasury's parked pot — and adds no fetch and no cost of its own; it excludes the execution/order-book `HOLDING` section, since the trading venues are not a Mandate. Free and local — no fetch of its own, reading whatever `data/treasury.yaml`, the portfolios and `data/altsignal/` already hold; a bad read costs a warning, not the digest. Skipped entirely on `--subject-only`, along with net worth, which the section feeds nothing into. |
| `digest --no-llm` | 🟢 | Every section except the roster narration. Completely free. |
| `review [name]` | 🟢 | What to do about positions you already hold, from `data/portfolios/<name>.yaml`. Reads the price cache and the stance store; no LLM, no network, places nothing. A holding with no cached price prints a row saying so rather than vanishing. Ends with a LEVELS section — what price is standing on and closing in on — which is independent of the roster and capped; `--levels` prints the uncapped scan. On a Plaid-synced account it also shows cash beside the ADDs, and prints a WRONG INSTRUMENT? banner above the table when our price and the broker's mark disagree by more than 25%. Also ends with an ALT-SIGNAL section (see `fetch-altsignal` below) — DefiLlama chain data beside a matching crypto holding, plus a MACRO block for the Kalshi/Polymarket markets in `cfg/altsignal.yaml`. **A YIELD line (#74) prints beside a holding whose asset has a cheaper-exposure wrapper in `cfg/altsignal.yaml`'s `wrappers:` block** — the same-asset receipt token with the highest APY that clears the Safety gate, plus what of it is already held; silent unless the book carries a readable contract address (`figi:`). |
| `fetch-altsignal` | 🟡 | Phase 5 alt-signal: DefiLlama (chain TVL/stablecoin supply/DEX volume, plus protocol-level fees/revenue/TVL/category/open-interest/unlock-schedule for the `protocols:` block, plus venue Safety-gate facts for the `venues:` and `wrappers:` blocks — see below), CoinGecko (protocol market cap/FDV/supply and, via `/derivatives/exchanges`, open interest and perps volume — the one source that reads both sides of a comparison the same way), Kalshi and Polymarket (hand-picked markets from `cfg/altsignal.yaml`), and pump.fun (recently graduated tokens, via Solana Tracker — needs `SOLANATRACKER_API_KEY`, see `.env.example`; skipped with a message if unset). Same tier as `fetch-funding` — free, third-party, rate-limitable — and read by `review` above for the chain rows and `compare` below for the protocol rows. pump.fun is logged but **not** surfaced anywhere: a brand-new graduating token confirms no holding or roster thesis, so it stays report-only (`--report`). **Venue Safety (#73), for the `venues:` block, and same-asset wrappers (#74), for the `wrappers:` block — one fetch covers both:** `/protocols` (audits, `forkedFromIds`, `hallmarks` — ~8.85 MB, one download per run), `/pools` (apy, sigma, count, outlier, stablecoin — ~11.37 MB, one download per run), and `/protocol/{slug}` (first non-zero `tvl[]` point, the honest deployment age — 2-30 MB per slug, **fetched once per slug, ever**, never re-polled once a `venue_first_tvl` row exists). ~20 MB total per run with both `venues:` rows configured, plus one one-time history fetch per newly seeded `wrappers:` slug. Read by `treasury` below via `core.safety`, and by `review` above via `review.yield_note`. |
| `compare <A> <B>` | 🟢 | Put one protocol beside another — the #66 comparison card. Reads `data/altsignal/` only, same policy as `review`: no fetch, no money, places nothing. Four cell states per metric (a value, no free source, not fetched, or categories differ for TVL) so a cell never silently reads as `0`, plus the three derived ratios (`mcap/OI`, `mcap/revenue`, `FDV/revenue`) and an age banner when either side's freshest reading is stale. `uv run fetch-altsignal` must run first to populate anything. |
| `dashboard` | 🟢 | The #84/#85 browser Surface: a JSON API over `review.cli.load_books` (ADR-0011's own wire shape, never a View's result object) plus the built `web/` front end. Binds `127.0.0.1` only, with no flag to widen it, and imports neither `execution` nor `llm` (ADR-0010) — it cannot spend and cannot sign. `--print-openapi` dumps the schema and exits without binding a port, which is how `scripts/gen-api-types.sh` regenerates `web/src/api/schema.d.ts` without a running server. **#93's Refresh button spawns four of the commands above as child processes** — `./scripts/data-pull.sh`, `fetch-prices`, `fetch-funding`, `fetch-altsignal` — never as an import, which is what keeps ADR-0010's "cannot spend and cannot sign" true: `.env` and its signing key load in the child, not this process. |
| `treasury` | 🟢 | What money is parked for its yield and what it earns — the #72 Treasury mandate. Reads `data/treasury.yaml` (the parked rows, hand-kept, outside `data/portfolios/`), every `data/portfolios/*.yaml` (for idle cash), and `data/altsignal/` (the Safety-gate facts `fetch-altsignal` last wrote); no fetch of its own and no network — every read is of something already on disk, places nothing. `amount`/`apy` per row are hand-typed, never re-priced — the total is typed principal, not net worth, and accrued yield since a row's `since:` date is not in it. Benchmarked against a `flat_rate` cash rate alone (ADR-0008); a missing file names `cfg/treasury.example.yaml` and exits 0, since nothing parked is a valid state. **Safety (#73):** a parked row whose `venue:` matches a `cfg/altsignal.yaml` `venues:` entry prints its gate result and score beside it — `UNCONFIGURED` (no such entry, e.g. an off-chain bank) prints nothing, never a false "no audit on record"; idle cash gets an ADVICE block of every gate-clearing, `stablecoin: true` pool, ranked by APY. |
...
| `plaid-sync [name ...]` | 🟢 / 💸 | Rewrites the `positions:` block of every linked portfolio from the broker, plus `cash:` and each row's `figi:`/`mark:`. Positions themselves are free and unmetered — Plaid bills per connection, not per call. **Not true any more for an account whose mandate declares a `held_flat` benchmark** (`retirement`, as of #65): that account also gets a transaction-history pull into `data/transactions/<name>.json`, and the first such pull against an Item starts a metered monthly Plaid subscription that cannot be cancelled without removing and re-linking the connection (~$0.17/month, one connection — spec #63). `--dry-run` still fetches for real; only the write is skipped. Refuses to write an empty positions list, so a broker hiccup cannot erase an account. Names every investment account the login reaches, including empty ones that would merge in if funded. Hand-kept portfolios are untouched and unmentioned. |
| `wallet-sync [name ...]` | 🟢 | The same job as `plaid-sync`, for self-custodied crypto: rewrites the `positions:` block of every portfolio carrying a `wallets:` block, from what the chain says the public addresses hold. **Default source is Zerion** (`oracle/zerion.py`) — indexes protocols, not just balances, so it also sees LP shares, lending deposits, and anything staked/locked that a plain ERC-20 scan cannot. Free but keyed and metered (Developer plan, 2,000 req/day, DeFi Positions bills against 25% of that). `--source alchemy` / `source: alchemy` in the file falls back to the original balance-only reader (`oracle/wallet.py`), free and unmetered per position. Stablecoins land in `cash:`, not as positions. Junk is dropped by price (no quote, or under `min_value:`) and every drop is printed. **The Alchemy path refuses to write on an incomplete read** — a chain that errored looks exactly like a chain you sold out of. **Zerion has no equivalent guard**: it answers with one call across every chain it indexes, so there is no per-chain signal to distrust if its view of one chain is stale — see `oracle/zerion.py`'s module docstring. Plaid-filled and hand-kept portfolios are untouched and unmentioned. |
