...
| `wallet-sync [name ...]` | 🟢 | The same job as `plaid-sync`, for self-custodied crypto: rewrites the `positions:` block of every portfolio carrying a `wallets:` block, from what the chain says the public addresses hold. Free — Alchemy's free tier, unmetered per position. Stablecoins land in `cash:`, not as positions. Junk is dropped by price (no quote, or under `min_value:`) and every drop is printed. **Refuses to write on an incomplete read** — a chain that errored looks exactly like a chain you sold out of. Plaid-filled and hand-kept portfolios are untouched and unmentioned. |
