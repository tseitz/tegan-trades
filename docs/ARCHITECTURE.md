...
| `plaid-sync [name ...]` | 🟢 | Rewrites the `positions:` block of every linked portfolio from the broker, plus `cash:` and each row's `figi:`/`mark:`. Free and unmetered — Plaid bills per connection, not per call, so nightly costs nothing. Refuses to write an empty list, so a broker hiccup cannot erase an account. Names every investment account the login reaches, including empty ones that would merge in if funded. Hand-kept portfolios are untouched and unmentioned. |
