...
- **The backlog is GitHub Issues, not a file in this repo.** `gh issue list` — see `docs/agents/issue-tracker.md`. It holds work you would *do*, not things you *learned*. Read it before starting new work; several issues are decisions already made but not yet executed. Close an issue when it's done. `docs/IMPROVEMENTS.md` was the old home and was imported wholesale on 2026-09-13; the ranked priority view is the pinned **Where to start** issue. **Comments across the repo still cite entries as `§n` — resolve them with `docs/agents/improvements-map.md`**, which also lists the refs that no longer resolve and the `§` that never meant this tracker.
  ...
  **Filing is the skills' job, and needs no permission.** `/wayfinder`, `/to-tickets` and `/triage` each write issues as their normal output; asking before every one stalls them. **This overrides §4b of `~/.claude/rules/common/development-workflow.md`** — that rule predates the Matt Pocock flow and its caution is now carried by the title test below instead. A finding you hit mid-task still isn't an issue; it goes where the next paragraph says.
