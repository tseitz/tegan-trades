...
- **The backlog is GitHub Issues, not a file in this repo.** `gh issue list` — see `docs/agents/issue-tracker.md`. It holds work you would *do*, not things you *learned*. Read it before starting new work; several issues are decisions already made but not yet executed. Close an issue when it's done. `docs/IMPROVEMENTS.md` was the old home and was imported wholesale on 2026-09-13; the ranked priority view is the pinned **Where to start** issue. **Comments across the repo still cite entries as `§n` — resolve them with `docs/agents/improvements-map.md`**, which also lists the refs that no longer resolve and the `§` that never meant this tracker.

  Labels: `status:open` · `status:partly-done` · `status:decided` · `status:watching` for state, and `corpus-supply` · `durability` · `venue-execution` · `routing` · `scoring` for theme. The five triage labels in `docs/agents/triage-labels.md` are a separate vocabulary, owned by `/triage`.

  **ASK BEFORE OPENING AN ISSUE — never file one unprompted.** When you find a real gap, defect, or better approach *while building something else*, raise it and default to proposing you fix it now. Only file it if I say to defer. Filing is not free: an issue *is* a decision to defer, and making that call silently is what grew the old file past 50 entries. "Write it down and keep going" was about not derailing and not dropping; it was never permission to choose deferral on my behalf.
  ...
  **The title test: if it reads as a statement rather than an instruction, it is a finding and does not belong here.** "On equities a stop is an intent, not a bound" is a fact about the world; it belongs beside the code that draws stops, where someone editing them will meet it. "Refuse a fill when the open has eaten the stop" is work. Findings filed as to-dos are read once by whoever triages the backlog and never by the person who needs them — and they are what makes entries long, because a fact needs explaining and a task does not.
  ...
  Two failure modes this has actually produced, both worth checking for: an issue that **duplicates documentation already in the code** and drifts from it (two entries were stale copies carrying *worse* numbers than the module they described), and **code comments citing an issue number**, which dead-end the moment that issue is correctly closed. Point at the module or the probe instead; those outlive the tracker.

  **An issue says what to do next and why it's worth doing — never what you learned getting there.** It must cite evidence (a measurement, a count, a real example) but must not *contain* it: one line of number, then a pointer. **Roughly 15 lines is the ceiling**; if an issue is growing past it, the excess is a finding and belongs somewhere else. Do not add "what we measured", "what was tried", "corrections to this issue", or a dated narrative of a session — that is what git history and the code are for.
...
- `oracle/wallet.py` + `wallet_cli.py` — **the same job for crypto you hold the keys to, and read-only more strongly than Plaid is.** A public address is the string you give someone to send you funds, so this path could not move a coin even in principle; it is a privacy leak, not a credential, which is why addresses live in the gitignored portfolio file and not in `cfg/`. CLI: `wallet-sync [name ...]` (nightly, free, touches only files carrying a `wallets:` block). There is no `wallet-link` — a chain needs no login. Stablecoins become `cash:` rather than positions, on the same reasoning that drops a broker's cash row: a dollar has no chart, and here it is also exactly what you could deploy. **Two contracts claiming one ticker are refused, never guessed between** — the obvious tie-breaks all lose, because a scam token mints nine billion units at $0.00002 and so outweighs the real position on both value and count; pin the real one with `prefer:`. Run `scripts/probe_alchemy_wallet.py` before assuming a chain is reachable, and probe the *name* before blaming the key: **Alchemy's docs call Solana `sol-mainnet` and the API only accepts `solana-mainnet`**, and the rejection reads as an entitlement problem, which sends you to the dashboard to enable something already enabled.
...
- **A cron job on a DigitalOcean droplet (`ssh tegan-droplet`, repo at `/root/tegan-trades`) runs the whole cycle once a day** (`scripts/nightly.sh`), fired daily at `0 11 * * *` (11:00 UTC) — see `crontab -l` on the droplet, not launchd. The laptop's launchd job is **uninstalled**; `scripts/nightly.sh` still carries the lid/battery/AC gate for that path, but it no-ops on Linux (`uname -s` != `Darwin`) and nothing installs it there anymore. **It spends no real money by default** — `ingest-x` is off since 2026-08-18 (`NIGHTLY_WITH_X` in the script carries the measurement behind that); `--with-x` restores it. `cat data/nightly.gate` says why it hasn't gone; `touch data/nightly.pause` stops it; see README for the rest. Assume the corpus may have moved since you last looked. The run ends with `digest`, which is **deliberately not a `step`** — it has to read the history row the step framework writes after the summary, so it can neither appear in that summary nor report on itself. **`digest` no longer runs `--vault` on the droplet** (fixed 2026-09-06) — its `/root/vault/Trading` is a bare local folder, not the real Obsidian vault, so the note was landing somewhere only reachable over SSH; email is the channel that actually reaches Tegan. `Trade Logs/Nightly.md`, written directly by `nightly.sh` rather than by `digest`, still lands in that same stranded folder — left as-is, see README's nightly section.
