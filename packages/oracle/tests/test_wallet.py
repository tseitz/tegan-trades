    ...
    *,
    ...
    return {
        ...
        "tokenPrices": (
            []
            if price is None
            else [{"currency": "usd", "value": price, "lastUpdatedAt": "now"}]
        ),
    }
...
def _rows(*tokens, min_value=wallet.MIN_VALUE_USD, staked=None):
    rows, skipped, cash, unpriced = wallet.rows_from(
        _read(*tokens), min_value=min_value, staked=staked
    )
    return rows, skipped, cash, unpriced
...
def test_an_emptied_token_is_not_news():
    rows, skipped, _, _ = _rows(_token(symbol="OLD", balance="0x0"))
    assert rows == ()
...
def test_an_unquoted_token_is_dropped_and_named():
    rows, skipped, _, _ = _rows(_token(symbol="SCAM", price=None, units=1_000_000))
    assert rows == ()
...
def test_a_position_under_the_floor_says_what_it_was_worth():
    rows, skipped, _, _ = _rows(
        _token(symbol="DUST", price="0.001", units=100), min_value=1.0
    )
    assert rows == ()
...
SCAM = (
    {"contract": "0xreal", "symbol": "LINK", "price": "15", "units": 100},
    {
        "contract": "0xfake",
        "symbol": "LINK",
        "price": "0.00002",
...
def test_two_tokens_claiming_one_ticker_are_refused_not_guessed_between():
    ...
    rows, skipped, _, _ = _rows(*(_token(**t) for t in SCAM))
    assert rows == ()
...
def test_the_richest_claimant_is_not_the_real_one():
    ...
    rows, _, _, _ = _rows(*(_token(**t) for t in SCAM))
    assert rows == ()
...
def test_prefer_settles_a_collision_by_contract_and_keeps_the_real_position():
    ...
    rows, skipped, _, _ = wallet.rows_from(
        _read(*(_token(**t) for t in SCAM)), prefer={"LINK": "0xreal"}
    )
...
def test_a_prefer_pointing_at_a_contract_you_do_not_hold_says_so():
    rows, skipped, _, _ = wallet.rows_from(
        _read(*(_token(**t) for t in SCAM)), prefer={"LINK": "0xneither"}
    )
    assert rows == ()
...
def test_a_wallet_holding_no_stablecoin_says_none_not_zero():
    ...
    _, _, cash, _ = _rows(_token(symbol="ARB", price="0.5", units=1000))
    assert cash is None
...
def test_a_yield_bearing_wrapper_stays_a_position():
    ...
    rows, _, cash, _ = _rows(_token(symbol="SUSDE", price="1.14", units=1000))
...
def test_the_contract_address_and_price_ride_along():
    ...
    rows, _, _, _ = _rows(
        _token(contract="0xc02aaa", symbol="WETH", price="2430.58", units=1)
    )
...
# ── staked SOL is the same exposure, not a new position — ADR-0008 ──


def _sol_native(units: float, price: str = "98.7"):
    return {
        "network": "solana-mainnet",
        "tokenAddress": None,
...
def test_staked_units_fold_into_the_liquid_sol_row_not_a_second_one():
    rows, _, _, unpriced = _rows(_sol_native(0.41), staked={"SOL": 2.34})
    assert [r.ticker for r in rows] == ["SOL"]
    assert rows[0].shares == pytest.approx(0.41 + 2.34)
    assert rows[0].staked == pytest.approx(2.34)
    assert unpriced == ()


def test_staked_units_create_a_row_when_liquid_sol_is_zero():
    """A wallet that staked everything reports zero liquid SOL, which `rows_from` would
    otherwise drop as an emptied token (trap 2) with no row and no skip line."""
    rows, _, _, unpriced = _rows(_sol_native(0.0), staked={"SOL": 2.34})
    assert [r.ticker for r in rows] == ["SOL"]
    assert rows[0].shares == pytest.approx(2.34)
    assert rows[0].staked == pytest.approx(2.34)
    assert unpriced == ()


def test_staked_units_create_a_row_when_liquid_sol_alone_is_under_the_floor():
    rows, skipped, _, _ = _rows(_sol_native(0.01), staked={"SOL": 2.34}, min_value=25.0)
    assert [r.ticker for r in rows] == ["SOL"]
    assert rows[0].shares == pytest.approx(0.01 + 2.34)
    assert not any(s.kind == "dust" for s in skipped)


def test_no_quote_on_the_native_row_reports_the_staked_ticker_unpriced_not_dropped():
    """No native row at all means no quote to value the staked units with. That is a failed
    read, not a position that silently vanished — the caller routes `unpriced` into `failed`."""
    rows, _, _, unpriced = _rows(staked={"SOL": 2.34})
    assert rows == ()
    assert unpriced == ("SOL",)


def test_a_collision_that_drops_the_sol_group_reports_it_unpriced_not_a_silent_loss():
    """`_fold` drops a whole ticker group on a price collision, same as it would for any
    scam token wearing a real symbol. When the group carries staked units, that drop must
    still surface as a failed read — the collision line alone would read as an ordinary
    dropped scam token, not as the staked position vanishing from the file."""
    rows, skipped, _, unpriced = _rows(
        _sol_native(0.41),
        # Priced well above the dust floor on its own, so it survives into `_fold` and the
        # collision actually fires — a scam token cheap enough to be dust would never reach
        # the code path this test exists to cover.
        _token(
            network="solana-mainnet",
            contract="ScamContract111",
            symbol="SOL",
            price="1.0",
            units=100,
        ),
    ...
    assert rows == ()
    assert unpriced == ("SOL",)
    assert any(s.kind == "collision" for s in skipped)
